...
% %figure; imshow(I);
% image=imread('20.png'); %26 25 24 23 22 17 12 8 7 5 4 3 2 %26 25 24 23 22 13 11 9 8 7 5 4 2 1
%
...
for count=41:45
    ...
    image=imread([num2str(count),'.png']);
    [m,n,k]=size(image);
    scale=360/m;
    image=imresize(image,scale);
    [m1,n1,k]=size(image);
    ...
    %
    % figure; imshow(Q);
    figure('units','normalized','outerposition',[0 0 1 1]);subplot(1,2,1); imshow(image);title('Approximate Estimation');
    ...
    subplot(1,2,2); imshow(image);title('Refined Estimation');
    % figure; imshow(Q);
